...
class TestAnalysis extends Analysis {
  constructor(id: AnalysisId, user: UserId) {
    super(id, user);
  }
}
...
describe('Analysis Entity', () => {
  ...
  it('Should create a valid Analysis Entity instance', () => {
    expect(new TestAnalysis(analysisId, userId)).toBeInstanceOf(TestAnalysis);
  });

  it('should be initialized with PENDING status', () => {
    const analysis = new TestAnalysis(analysisId, userId);
